...
class MarketClock:
    ...
    def entries_allowed(self, now: datetime) -> bool:
        ...
        return local.weekday() < 5 and self.settings.entry_start <= local.time() < self.settings.entry_cutoff

    def force_exit_due(self, now: datetime) -> bool:
        local = now.astimezone(self.settings.timezone)
        return local.weekday() < 5 and local.time() >= self.settings.force_exit
