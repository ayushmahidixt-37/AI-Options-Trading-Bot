    ...
    with archive.connect() as con:
        cursor = con.execute(
            """INSERT INTO range_usage(
                   ...
                   range_start, range_end, outcome_label, forced_override_reason, git_commit, notes
               ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                recorded_at,
                candidate_name,
...
def export_ledger_json(archive: MarketArchive, target: str | Path) -> Path:
    ...
    context = research_context_for_evaluation(archive)
