"""#11 Short Strangle -- 9:20, NIFTY50, community code, spec item #11.

Identical to s10_short_straddle_920.py except the strikes: this sells a
fixed 50-point-wide strangle (Call at ATM+50, Put at ATM-50) instead of the
ATM straddle. Ported from the community "0920_short_strangle" pattern
(buzzsubash/algo_trading_strategies_india), run every real trading day
against that day's nearest weekly expiry.

Spec implemented:
  Entry     09:20 -- sell Call at (ATM+50) and Put at (ATM-50), ATM = spot
            LTP rounded to nearest 50 (`E.atm_strike(spot, 50)`)
  Stop      25% above sell price on EACH leg independently, PLUS a 5-point
            ...
            substitute for it)
  Re-entry  if BOTH legs have been stopped out, re-enter a fresh
            (ATM+50)/(ATM-50) strangle at 12:30 (one re-entry only, same day)
...
def main(argv=None):
    ...
    for day in days:
        ...
        # size_or_floor wants risk PER UNIT (per share/point), not per lot --
        # it multiplies by lot_size itself internally. Fixed during the
        # portfolio-sizing follow-up pass (was passing an already-per-lot
        # value here, which double-divided by lot_size and floored far more
        # often, and more severely, than the real 1-2% budget required).
