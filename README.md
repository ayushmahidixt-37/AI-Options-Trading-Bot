...
During the open session, the next archive phase collects closed five-minute candles for the nearest-expiry NIFTY CE and PE contracts at ATM ±5 strikes. Collection is limited to once per five-minute bucket and retains expired-contract history. The offline backtest button uses only local strategy observations, instrument metadata, and option candles: an entry uses the next available option candle open and an exit uses the last candle before the next opposing directional signal. Results are preliminary gross option points, not guaranteed returns; no network request or order is made by the backtester. It reports insufficient data until enough matching option history has accumulated.
