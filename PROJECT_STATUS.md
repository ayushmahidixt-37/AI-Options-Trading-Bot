...
**2026-08-29 — a smaller, independent finding from the original 7-month
archive: a consistent worst-hour pattern, and a new "Skip 10:25-11:25"
candidate.** Done in parallel with, and using an older/smaller archive
...
window). The resulting "Skip 10:25-11:25" candidate beat Baseline on
both development (+23,437.85 vs +20,517.50) and validation (+2,943.25
vs -626.80, lower drawdown) on that older archive's Jan-Mar/Apr-May
split; no test leg was available on that archive at the time. Labeled
**Open**, not Confirmed or Exploratory. Now that the archive has since
been extended by years via DhanHQ (see below), re-testing this specific
candidate against a genuinely fresh period there is a natural, cheap
follow-up. See `BACKTEST_FINDINGS.md`'s 2026-08-29 entry for the full
per-strategy table.

**2026-08-25 — a fine-tuning pass on Candidate B, both items resolved "no change needed."**
